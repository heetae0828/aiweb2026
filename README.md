...
| **일정 자동 생성** | 선택한 기간(당일치기~4박5일)에 맞춰 1일차/2일차... 형식으로 시간대별 일정표 생성 |
| **PDF 다운로드** | 생성된 일정표를 한글 폰트 적용 PDF로 다운로드 |
...
- **단일 API 호출**: 추천과 일정을 분리된 두 번의 API 호출 대신, 하나의 구조화된 JSON 프롬프트로 모든 결과를 한 번에 수신합니다. 비용과 응답 속도를 모두 절감합니다.
...
| `_build_prompt()` | 멤버 정보·조건을 바탕으로 GPT 프롬프트 생성 |
| `_schedule_html()` | GPT 응답의 schedule JSON → 일차별 HTML 테이블 |
...
앱 코드에서 `os.getenv("OPENAI_API_KEY")`로 읽기 때문에 별도 코드 수정 없이 동작합니다.
